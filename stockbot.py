...
def get_stock_info(stock):
    n = randint(1, 2)
    url = "https://query{}.finance.yahoo.com/v8/finance/chart/{}?region=US&lang=en-US&includePrePost=false&interval=1d&range=1d&corsDomain=finance.yahoo.com&.tsrc=finance".format(
        n, stock)
    ...
    headers = {
        ...
        'method': 'GET',
        'scheme': 'https',
        'path': '/v8/finance/chart/{}?region=US&lang=en-US&includePrePost=false&interval=1d&range=1d&corsDomain=finance.yahoo.com&.tsrc=finance'.format(
            stock),
    ...
    try:
        ...
        print('Yahoo API CONNECTION ERROR: {}'.format(e))
...
def get_no_of_shares(symbol):
    try:
        ...
        print('{}: CONNECTION ERROR: {}'.format(symbol, e))
        time.sleep(randint(1, 3))
        get_no_of_shares(symbol)


def alpaca_order(symbol, side, _type='market', time_in_force='day'):
    num_shares = get_no_of_shares(symbol)
    try:
        api.submit_order(
            symbol=symbol,
            qty=num_shares,
...
def get_todays_picks(strong_buy_stocks, tradealgo="moved"):

    stock_info =[]
    for stock_item in strong_buy_stocks:
        stock = stock_item['symbol'].upper()
        # print('DEBUG', stock)
        sys.stdout.write('.')
        sys.stdout.flush()
        ...
        weightage = ((stock_item['moved'] - change_low_to_market) * 3) + change_low_to_market

        stock_info.append({'symbol': stock, 'company': stock_item['company'], 'market_price': stock_price,
                           'low': stock_low, 'high': stock_high, 'volume': stock_volume,
                           'moved': stock_item['moved'], 'change_low_to_market': change_low_to_market,
...
Alpaca algo stock trading bot.""".format(STOCKBOT_VERSION)
    ...
    while True:
        try:
            ...
            stock_picks = get_todays_picks(strong_buy_stocks, tradealgo)
            ...
            if datetime.today().weekday() in [0, 1, 2, 3, 4] \
                    and datetime.now(tz=TZ).hour == buy_sh \
                ...
                while True:
                    for stock in stock_picks:
                        ...
                        if startbuytime == 'buyatclose':
                            ...
                            n = 5
                        buy_price = stock_price_buy * get_no_of_shares(stock['symbol'])
                ...
                while True:
                    for stock in bought_stocks:
                        ...
                        if change_perc >= SELL_PERCENT_GAIN:
                            ...
                            sell_price = stock_price_sell * get_no_of_shares(stock['symbol'])
                            stock_data = get_stock_info(stock['symbol'])
                            stock_vol_now = stock_data['chart']['result'][0]['indicators']['quote'][0]['volume'][0]
